 ...
 *        ticks.map { bitmap.flipTick(it.toBigInt(), spacing.toBigInt()) }.aggregate(),
 *        ticks.map { bitmap.isTickSet(it.toBigInt(), spacing.toBigInt()) }.aggregate(),
 *        router.quote("100000".toBigInt(), "12415235134".toBigInt(), "982341485157841".toBigInt())
 *    )
 *
 *    // execute the call
 ...
 *    val swapResult = response.getAs<BigInt>(2)
 * ```
 * */
class Multicall3(
    provider: Middleware,
    ...
    fun aggregate(vararg calls: Call): PayableFunctionCall<AggregateResult> = PayableFunctionCall(
        ...
        AggregateResult(data[0] as BigInt, data[1] as List<Bytes>)
    }
    ...
    fun getBasefee(): ReadFunctionCall<BigInt> = ReadFunctionCall(
        this.provider,
        this.address,
    ...
    fun getBlockHash(blockNumber: BigInt): ReadFunctionCall<Bytes> = ReadFunctionCall(
        this.provider,
        this.address,
    ...
    fun getBlockNumber(): ReadFunctionCall<BigInt> = ReadFunctionCall(
        this.provider,
        this.address,
    ...
    fun getChainId(): ReadFunctionCall<BigInt> = ReadFunctionCall(
        this.provider,
        this.address,
    ...
    fun getCurrentBlockDifficulty(): ReadFunctionCall<BigInt> = ReadFunctionCall(
        this.provider,
        this.address,
    ...
    fun getCurrentBlockGasLimit(): ReadFunctionCall<BigInt> = ReadFunctionCall(
        this.provider,
        this.address,
    ...
    fun getCurrentBlockTimestamp(): ReadFunctionCall<BigInt> = ReadFunctionCall(
        this.provider,
        this.address,
    ...
    fun getEthBalance(addr: Address): ReadFunctionCall<BigInt> = ReadFunctionCall(
        this.provider,
        this.address,
    ...
    companion object {
        ...
        private var INSTANCE_PER_CHAIN_ID = HashMap<Long, Multicall3>()
        ...
        fun getInstance(provider: Middleware): Multicall3 {
            var existing = INSTANCE_PER_CHAIN_ID[provider.chainId]
            if (existing != null) {
                return existing
            }

            // do a copy-on-write when creating a new instance - this is fine because 99% of the time, a cached
            // instance will already exist, avoiding synchronization overhead
            return synchronized(INSTANCE_PER_CHAIN_ID_LOCK) {
                existing = INSTANCE_PER_CHAIN_ID[provider.chainId]
                if (existing == null) {
                    existing = Multicall3(provider, getAddressForChainId(provider.chainId))
                    val new = HashMap(INSTANCE_PER_CHAIN_ID)
                    new[provider.chainId] = existing
                    INSTANCE_PER_CHAIN_ID = new
                }
                existing
            }
        }
 ...
 * */
private inline fun <T> List<Multicall3.Aggregatable<*>>.withDataAndValue(consumer: (Bytes, BigInt?) -> T): T {
    if (this.isEmpty()) {
    ...
    for (i in this.indices) {
        val call = this[i]
        if (call.value != null && call.value != BigInts.ZERO) {
            anyPayable = true
            break
        }
    ...
    return when {
        anyPayable -> {
            var totalValue = BigInts.ZERO
            val arr = List(this.size) {
                val req = this[it]
                val value = req.value ?: BigInts.ZERO
                totalValue += value
